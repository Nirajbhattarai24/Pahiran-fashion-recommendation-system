...
sub_model = tf.keras.models.load_model('D:\Project\PAHIRAN\Pahiran-wardobe-management-system\model\model_sub')
top_model = tf.keras.models.load_model('D:\Project\PAHIRAN\Pahiran-wardobe-management-system\model\model_top')
bottom_model = tf.keras.models.load_model('D:\Project\PAHIRAN\Pahiran-wardobe-management-system\model\model_bottom')
foot_model = tf.keras.models.load_model('D:\Project\PAHIRAN\Pahiran-wardobe-management-system\model\model_foot')
...
# # Load pre-trained models and define lists (existing code)
# EMBEDDING_DIMENSION = 128

# # Initialize Annoy index
# annoy_index = AnnoyIndex(EMBEDDING_DIMENSION, 'angular')  # 'angular' metric for cosine similarity

# # Function to create embeddings for clothing items
# def create_embedding(image_path):
#     # Load and preprocess the image (existing code)
#     img = preprocess_image(image_path)
#     # Extract embeddings using the pre-trained models (existing code)
#     embeddings = extract_embeddings(img)
#     return embeddings


# # Build Annoy index
# def build_annoy_index(image_paths):
#     for i, path in enumerate(image_paths):
#         # Create embedding for the clothing item
#         embedding = create_embedding(path)
#         # Add the clothing item's embedding to the Annoy index
#         annoy_index.add_item(i, embedding)
#     # Build the index
#     annoy_index.build(NUM_TREES)

# # Function to find similar clothing items
# def find_similar_items(query_image_path, num_results=5):
#     # Create embedding for the query image
#     query_embedding = create_embedding(query_image_path)
#     # Query the Annoy index for similar items
#     similar_item_indices = annoy_index.get_nns_by_vector(query_embedding, num_results)
#     # Retrieve paths of similar items
#     similar_item_paths = [image_paths[i] for i in similar_item_indices]
#     return similar_item_paths

# # Update the single_classification function to return the clothing item's embedding
# def single_classification(single_path):
#     # Load and preprocess the image (existing code)
#     img = preprocess_image(single_path)
#     # Extract embeddings using the pre-trained models (existing code)
#     embeddings = extract_embeddings(img)
#     return embeddings